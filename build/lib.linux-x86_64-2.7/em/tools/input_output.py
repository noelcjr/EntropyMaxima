        ...
        for i in sp.get_models():
            for j in i.get_chains():
                ...
                for c in inserts_report:
                    print(c)
    def fix_pdb_from_CHARMM(self,a=21,b=72):
        ...
        for i in contents_list:
            outFile.write(i+'\n')
        outFile.close()
    def write_pdb(self,SS,basedir,filename,output='original'):
        ...
        for g in SS.models:
            ...
            if output == 'original':
                for h in range(SS.Full_Structure.shape[0]):
                    if SS.Full_Structure.loc[h,'atminfo'+g]:
                        ...
                        int(SS.Full_Structure.loc[h,'aaid']),'',SS.Full_Structure.loc[h,'x'+g],\
                        SS.Full_Structure.loc[h,'y'+g],SS.Full_Structure.loc[h,'z'+g],\
                        SS.Full_Structure.loc[h,'occupancy'],SS.Full_Structure.loc[h,'B_factor'],\
            ...
            for i in lines:
                outFile.write(i+'\n')
            outFile.close()
    def prepare_pdb_for_charmm(self, optionspdbin,optionscrdout,optionsseqfix):
        if not os.path.exists(optionspdbin):
            print "Error: No --pdbin option entered. Type -h or --help."
            sys.exit(1)
        pdb_parser = struct.PDBParser(QUIET = True)
        #sp = pdb_parser.get_structure('Costruct', file_path)
        sp = pdb_parser.get_structure('Costruct', optionspdbin)
        # This fixes the problem of CHARMM not generating a chain id.
        # But needs to be tested in more systems
        print("For pdb=",optionspdbin)
        ...
        # TODO: IONS chain identifier are not changed and remain the same as
        #       in the original PDB. This is ok for now but there might be
        #       special cases where it may be a problem
        if optionscrdout == None:
            optionscrdout = 'out_'+optionspdbin
            print("WARNING: No --crdout given. The following is the output file:",optionscrdout)
        else:
            '''    
        ...
        if optionsseqfix == None:
            optionsseqfix = 'out_'+optionspdbin
            print("WARNING: No --seqfix given. No files will be generated.")
        else:
            text = optionsseqfix
            if text.lower() == 'yes':
                ...
                for i in sp.get_chains():
                    ...
                    for j in sp.get_residues():
                        if j.get_parent().id == i.id:
                            ...
                            outFile.write('rename resid x'+str(count)+' sele segid '+i.id+' .and. resid '+str(count)+' end\n')
                            count += 1
                            residue_count += 1
                    ...
                    for j in sp.get_residues():
                        if j.get_parent().id == i.id:
                            outFile.write('rename resid '+str(offset+count)+' sele segid '+i.id+' .and. resid x'+str(count)+' end\n')
                            count += 1
                    outFile.write('! Second pass complete.\n')
                    outFile.write('\n')
                    outFile.write('! End of generated script\n')
                    outFile.close()
            print(' WARNING: Check output if Ions are present in the original pdb structure.')
            print('          Check INF and SEQ files for systems with missing amino acids.')
            print('          Modifications to the code might be required.')
    def min_max(self,optionspdbin3):
        pdb_parser = struct.PDBParser(QUIET = True)
