...
from em.code.structure import crd,psf
...
# This class manages the search for a linker or loop in search of some outcome. It could be moved later if it is too
        ...
        for i in range(len(idx_ss.comp_indx)):
            if idx_ss.comp_indx[i][3] == 'PRO':
                if idx_ss.comp_indx[i][4] == 'AMIN':
                    ...
                    new_index.extend(temp_list)
                elif idx_ss.comp_indx[i][4] == 'SIDE':
                    new_index.extend([k for k in range(idx_ss.comp_indx[i][5],idx_ss.comp_indx[i][6]+1)])
                elif idx_ss.comp_indx[i][4] == 'CARB':
                    new_index.extend([k for k in range(idx_ss.comp_indx[i][5],idx_ss.comp_indx[i][6]+1)])
            else:
                #print("MMGBSA_L 383 ",i,insu.Full_Structure.columns)
                #print("MMGBSA_L 383 ",i,insu.Full_Structure.loc[9:20, ['aa', 'aaid', 'ent_id']])
                #print("MMGBSA_L 383 ",i)
                #print(insu.Full_Structure.head())
                new_index.extend([k for k in range(idx_ss.comp_indx[i][5],idx_ss.comp_indx[i][6]+1)])
                #print("MMGBSA_L 385 ",i)
        print(insu.Full_Structure.head())
        print(insu.Full_Structure.tail()) 
        print(insu.Full_Structure.dtypes)
